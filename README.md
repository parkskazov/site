<!--О проекте-->
# 📱 AR-WEB Park Skazov

![Logotype](https://github.com/user-attachments/assets/f4eca4dd-99f2-46d2-8d61-8a319316588c)



<!--Блок информации о репозитории в бейджах--> 
[![Platform](https://img.shields.io/badge/Platform-Android-green?style=flat&logo=android)](https://developer.android.com/)
[![Platform](https://img.shields.io/badge/Platform-iOS-lightgrey?style=flat&logo=apple)](https://developer.apple.com/ios/)
[![License](https://img.shields.io/badge/License-MIT-yellow?style=flat)](https://github.com/LubninEgor/AR_Pervouralsk_Android/blob/main/LICENSE)
![Made With Love](https://img.shields.io/badge/Made%20With%20Love%20by%20Cheloveki%20team-FF69B4?style=flat)
![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)

# Квест-экскурсия с AR для Парка Сказов город Арамиль

**AR-квест** с **точным распожнаванием меток (маркеров)** на базе **GitHub Padges** и **AR.js**. Проект позволяет проходить квест с **3D-моделями** сказочных героев **разрадывать их загадки** через **камеру телефона** и **AR-технологии**. Поддерживаются **ЛЮБЫЕ устройства как Android, так и IOS**.

## ⚡ Быстрый старт

bash
``` git clone https://github.com/parkskazov/site.git```

<!--Установка-->
## 🚀 Как запустить проект ? (полное руководство)

Установка:

1.   Клонируйте репозиторий:

(через команду) 
```git clone https://github.com/parkskazov/site.git```
			
   (или через сайт GutHub https://github.com/parkskazov/site)
			
2.   Основа - Index.html, css и js прописаны там же

## 📌 Возможности


 - Распознавание целевых меток (5 штук, представлены ниже. В виде черно-белых квадратов) в браузере, без скачивания приложения (это для команды большая победа!!!)
 * ВАЖНО: оставлять светлый (рекомендуется белый) фон вокруг черного квадрата каждой метки, минимум 2 см (~ 1/4 от чернойго квадрата)

 - Поддержка Android и IOS
 
 - Добавлена защита от читерства (можно получить только 1 приз за 24 часа)
	
<!--лицензия-->
## 📝 Лицензия [MIT](LICENSE)

Проект сводобный, но мы просим вас помнить и об авторах проекта.

❤️ Вставьте всего пару строчек о нас в ваш проект ❤️

авторы : https://t.me/Prpr11 и https://t.me/Lybnineg

репозиторий в GitHub https://github.com/parkskazov/site

## 🌟 Roadmap
 ✅Базовая настройка AR и меток  
 ✅Добавление моделей 
 ✅✅✅Поддержка iOS

## 🤝 Как помочь проекту
- Предлагайте идеи через [Issues](https://github.com/parkskazov/site/issues).  
- Форкайте репозиторий и присылайте Pull Requests.  
- Тестируйте приложение и сообщайте о багах.

<!--Приложение-->
## 📸 Скриншоты/Видео

<img width="400" height="600" alt="Frame 135 (3)" src="https://github.com/user-attachments/assets/184e2c53-d2e8-4cd6-ad60-5485e3aa5201" />

<img width="280" height="600" alt="photo_2026-06-30_09-12-56" src="https://github.com/user-attachments/assets/37bf7393-1e4d-42d4-b330-71f14035ccaf" />





## 📞 Контакты

<div align="center">
</div>

### 👨‍💻 Команда разработки

<table>
  <tr>
    <td align="center">
      <a href="https://github.com/LubninEgor">
        <img src="https://avatars.githubusercontent.com/u/114929277?s=100" width="100px;" alt="Егор Лубнин"/>
        <br />
        <sub><b>Егор Лубнин</b></sub>
      </a>
      <br />
      <br />
      <a href="https://t.me /Lybnineg" title="Telegram">
        <img src="https://img.shields.io/badge/Telegram-Lybnineg-blue?logo=telegram" />
      </a>
    </td>
	   <p> </p>
    <td align="center">
      <a href="https://github.com/Hii1111">
        <img src="https://avatars.githubusercontent.com/u/182222788?s=100" width="100px;" alt="PRPR11"/>
        <br/>
        <sub><b>Вячеслав Глазунов</b></sub>
      </a>
      <br />
      <br />
      <a href="https://t.me/Prpr11" title="Telegram">
        <img src="https://img.shields.io/badge/Telegram-Prpr11-blue?logo=telegram" />
      </a>
    </td>
  </tr>
